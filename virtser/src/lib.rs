...
use std::{fs::File, io::{Read, Write}, os::fd::{AsRawFd, FromRawFd, IntoRawFd, OwnedFd, RawFd}, pin::{pin, Pin}, time::Duration};
use nix::{errno::Errno, pty::{openpty, OpenptyResult}, sys::termios::{self, BaudRate, ControlFlags, LocalFlags, SetArg}};
mod error;
...
impl VirtSer {
    pub fn new() -> Result<Self> {
        ...
        set_parity(&slave_file, Parity::None)?;
    ...
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        loop {
            match self.master_file.write(buf) {
                Err(err) if err.kind() == std::io::ErrorKind::WouldBlock => {std::thread::sleep(Duration::from_millis(100));continue;},
                other => {return other;}
            }
        }
    }
...
fn set_parity(file: &File, parity: Parity) -> Result {
    let mut termio = termios::tcgetattr(file)?;
    println!("Got termios");
    match parity {
        Parity::None => termio.control_flags &= !ControlFlags::PARENB,
        Parity::Even => {
            termio.control_flags |= ControlFlags::PARENB;
            termio.control_flags &= !ControlFlags::PARODD;
        }
        Parity::Odd => termio.control_flags |= ControlFlags::PARENB | ControlFlags::PARODD,
    }
    println!("Sat termios");
    termios::tcsetattr(file, SetArg::TCSANOW, &termio)?;
    println!("Set termios");
